...
try{
	...
	Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/web_clinic",
	        "root", "root");
	 dataupdate = con.prepareStatement("update user_reg set name = ?, address=?, phoneno=?, email=?, imgurl=? where email = '"+useremail+"'");
...
try{	int a = 1;
	   ...
	   if ( !fi.isFormField () ) { 
		  ...
		  if( fileName.lastIndexOf("\\") >= 0 )
		   ...
		   dataupdate.setString(5, img_url);
	...
	//dataupdate.setString(1, request.getParameter("email"));
	//dataupdate.setString(2, request.getParameter("name"));
	//dataupdate.setInt(3, Integer.parseInt(request.getParameter("age")));
	//dataupdate.setString(4, request.getParameter("address"));
	//dataupdate.setDouble(5, Double.parseDouble(request.getParameter("phoneno")));
	
	//regupdate.setString(1, request.getParameter("email"));
	//regupdate.setString(2, request.getParameter("name"));
	...
	//if(a>0 && b>0){ 
					...
					//response.sendRedirect("userviewprofile.jsp");}
